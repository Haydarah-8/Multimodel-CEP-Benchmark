# Figure companion writer (maintainer source)

The canonical Python for `scripts/write_figure_companions.py` lives in the fenced block below. Regenerate the runnable script from `llm-safety-experiment/` with:

    python scripts/sync_figure_companion_writer.py

(`sync_figure_companion_writer.py` copies the fenced block into `scripts/write_figure_companions.py`.)
